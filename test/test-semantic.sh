...
TEST_DIR="test/test"
LOG_FILE="logs/Test_Semantic-$(date +'%d%m%Y-%H%M').log"
...
for category in "${CATEGORIES[@]}"; do
    ...
    for test_case_dir in "$TEST_DIR/$category"/*/; do
        ...
        output=$(java -cp /app/antlr-4.13.2-complete.jar:/app/src:/app/app app.Main "$agu_file" --test-semantic "$MAX_ERRORS" 2>&1)
        result=$?
        ...
        elif [ "$category" == "invalid-semantic" ]; then
            if [ $result -ne 0 ]; then
                echo "✅ $test_name"
                ((passed++))
                ((semantic_passed++))
